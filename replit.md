...
- **No ORM**: Raw pg pool for simplicity; schema created via `initDB()` with `CREATE TABLE IF NOT EXISTS`
...
- **Proposals**: Create → AI-generate → edit with Tiptap → send via email → client accepts with e-signature
...
- **Settings**: Business profile (name, color, currency) + Pro upgrade (Stripe)
- **Public page**: Branded proposal view with accept/decline/comment flow
...
- `puppeteer` is in package.json but not yet used (reserved for PDF export feature)
